...
sftp = ssh.open_sftp()
sftp.putfo(open("/tmp/ljbljb.service", "w"), "/etc/systemd/system/ljbljb.com-backend.service")
sftp.close()

stdin, stdout, stderr = ssh.exec_command("cat > /etc/systemd/system/ljbljb.com-backend.service << 'EOF'\n" + service_config + "EOF")
stdout.read()
print("✓ 服务配置已更新")
